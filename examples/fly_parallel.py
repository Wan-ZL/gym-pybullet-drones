...
def parallel_control(action_paral, j, ctrl, CTRL_EVERY_N_STEPS, env, obs, TARGET_POS, wp_counters, INIT_XYZS, INIT_RPYS):
    action_paral[str(j)], _, _ = ctrl[j].computeControlFromState(control_timestep=CTRL_EVERY_N_STEPS * env.TIMESTEP,
    ...
    for i in range(0, int(ARGS.duration_sec*env.SIM_FREQ), AGGR_PHY_STEPS):
        ...
        if i%CTRL_EVERY_N_STEPS == 0:
            ...
            for j in range(ARGS.num_drones):
                t = Process(target=parallel_control, args=[action_paral, j, ctrl, CTRL_EVERY_N_STEPS, env, obs, TARGET_POS, wp_counters, INIT_XYZS, INIT_RPYS])
                threads.append(t)
                t.start()
            for t in threads:
                t.join()
            ...
            for j in range(ARGS.num_drones):
                action[str(j)] = action_paral[str(j)]
